...
<regprof:regprofDocExamAttachmentEdit>
	<v:form action="${pathPrefix }/control/doc_exam_attachment/save"
		method="post" name="form_doc_exam_attachment"
		onsubmit="startProgressUpdater();" enctype="multipart/form-data"
		backurl="${backUrlDocExam }#examination_document_field" addGenerateButton="${id == 0}"> <!-- temporary solution -->
		...
		<c:if test="${id!=0 }">
			...
			<a class="flt_rgt" href="${pathPrefix }/control/doc_exam_attachment/${fileName }?id=${id }&amp;original=1" 
				target="_blank">Свали прикачения файл</a>
			<div class="clr"><!--  --></div>
			<c:if test="${scannedFileName != null }" >
				<a class="flt_rgt" href="${pathPrefix }/control/doc_exam_attachment/${scannedFileName }?id=${id }" 
		...
		<p>
			...
			<nacid:combobox id="docTypeId" name="docTypeId" disabled="${docflowNum}" style="brd" attributeName="docTypeCombo" />
		</p>
		<div class="clr"><!--  --></div>
...
<div style="text-align: center;">
	<a target="_blank" href="${pathPrefix }/control/doc_exam_attachment/${scannedFileName }?id=${id }">
		<img src="${pathPrefix }/control/doc_exam_attachment/view?width=500&id=${id }" />
	</a>
</div>
</c:if>
