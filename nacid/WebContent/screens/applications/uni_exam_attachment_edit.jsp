...
<nacid:uniExamAttachmentEdit>
	<v:form action="${pathPrefix }/control/uni_exam_attachment/save"
		method="post" name="form_uni_exam_attachment"
		onsubmit="startProgressUpdater();" enctype="multipart/form-data"
		backurl="${backUrlUniExam }" addGenerateButton="${id==0}">
		...
		<c:if test="${id!=0 }">
			...
			<a class="flt_rgt" href="${pathPrefix }/control/uni_exam_attachment/${fileName }?id=${id }&amp;original=1"
				target="_blank">Свали прикачения файл</a>
			<div class="clr"><!--  --></div>
			<c:if test="${scannedFileName != null }" >
				<a class="flt_rgt" href="${pathPrefix }/control/uni_exam_attachment/${scannedFileName }?id=${id }" 
		...
		<input id="applGroup" type="hidden" name="applGroup" value="${groupName }" />
		<input id="id" type="hidden" name="id" value="${id }" />
		<input id="universityValidityId" type="hidden" name="universityValidityId"
			value="${universityValidityId }" />
		...
		<p>
			...
			<nacid:combobox id="docTypeId" name="docTypeId" disabled="${docflowNum}"
					attributeName="docTypeCombo" style="brd"/>
		</p>
		<div class="clr"><!--  --></div>
...
<div style="text-align: center;">
	<a target="_blank" href="${pathPrefix }/control/uni_exam_attachment/${scannedFileName }?id=${id }">
		<img src="${pathPrefix }/control/uni_exam_attachment/view?width=500&id=${id }" />
	</a>
</div>
</c:if>
