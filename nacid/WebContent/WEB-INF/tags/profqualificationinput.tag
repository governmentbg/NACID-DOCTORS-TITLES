...
<div id="flat_nomenclature_${id}">
	...
	<span id="new_flat_nomeclature_${id}" style="display:none; padding: 20px; width: 825px; height: auto;">
		<a href="javascript:void(0);" style="margin-right: 10px;" onclick="saveProfQualification('${id}', '${onsuccess}');">Запис</a>
...
new Autocomplete($('${id}'), { 
  	...
  	onSelect: updateSecProfQualificationId,
  	width:500
});

function updateSecProfQualificationId(value, data) {
	var id = '${id}';
...
function saveProfQualification(id, onSuccess) {
	if (validateText($(id), true, 2, 200, null, '') == true) {
		new Ajax.Request('/nacid_regprof/control/sec_prof_qualification_ajax/save', {
	  		method: 'post',
