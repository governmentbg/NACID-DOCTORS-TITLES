...
<regprof_ext:extApplicationReportView attributePrefix="${attributePrefix }">
		<fieldset><legend class="noForm">Данни от заявлението</legend>
            <regprof_ext:extApplicationViewReportPerson>
                ...
                <nacid_ext:report_subelement name="Място на раждане" value="${person_birth_country}"/>
                <nacid_ext:report_subelement name="Населено място" value="${person_birth_city}"/>
                <nacid_ext:report_subelement name="Дата на раждане" value="${personBirthDate}"/>
       ...
       <regprof_ext:extApplicationReportTrainingCourseView attributePrefix="${attributePrefix}">
	       		...
	       		<fieldset><legend class="noForm">Информация за курса на обучение</legend>
	       			<c:if test="${sec}">
	       				<fieldset><legend class="noForm">${education}</legend>
	       					...
	       					<nacid_ext:report_subelement name="Обучаваща институция(ново)" value="${prof_institution_name_txt}"/>
	       					<nacid_ext:report_subelement name="Обучаваща институция старо име" value="${prof_institution_org_name}"/>
	       					<nacid_ext:report_subelement name="Обучаваща институция старо име(ново)" value="${prof_institution_org_name_txt}"/>
	       					<nacid_ext:report_subelement name="Професионална квалификация по документи" value="${sec_prof_qualification}"/>
	       					<nacid_ext:report_subelement name="Професионална квалификация по документи(ново)" value="${sec_prof_qualification_txt}"/>
	       					<regprof_ext:extApplicationSpecialitiesView attributePrefix="${attributePrefix}">
	       						<nacid_ext:report_subelement name="Специалност" value="${sec_speciality}"/>
	       						<nacid_ext:report_subelement name="Специалност(ново)" value="${sec_speciality_txt}"/>
	       					</regprof_ext:extApplicationSpecialitiesView>
	       			...
	       			<c:if test="${high || sdk}">
	       				<fieldset><legend class="noForm">Висше образование</legend>
	       					...
	       					<nacid_ext:report_subelement name="Професионална квалификация по документи(ново)" value="${high_prof_qualification_txt}"/>
	       					<regprof_ext:extApplicationSpecialitiesView attributePrefix="${attributePrefix}">
	       						<nacid_ext:report_subelement name="Специалност" value="${high_speciality}"/>
	       						<nacid_ext:report_subelement name="Специалност(ново)" value="${high_speciality_txt}"/>
	       					</regprof_ext:extApplicationSpecialitiesView>
	       			...
	       			<c:if test="${sdk}">
	       				<fieldset><legend class="noForm">СДК</legend>
       						...
       						<nacid_ext:report_subelement name="Обучаваща институция(ново)" value="${sdk_prof_institution_name_txt}"/>
       						<nacid_ext:report_subelement name="Обучаваща институция старо име" value="${sdk_prof_institution_org_name}"/>
       						<nacid_ext:report_subelement name="Обучаваща институция старо име(ново)" value="${sdk_prof_institution_org_name_txt}"/>
	       					<nacid_ext:report_subelement name="Професионална квалификация по документи" value="${sdk_prof_qualification}"/>
	       					<nacid_ext:report_subelement name="Професионална квалификация по документи(ново)" value="${sdk_prof_qualification_txt}"/>
	       					<regprof_ext:extApplicationSpecialitiesView attributePrefix="${attributePrefix}">
	       						<nacid_ext:report_subelement name="Специалност" value="${sdk_speciality}"/>
	       						<nacid_ext:report_subelement name="Специалност(ново)" value="${sdk_speciality_txt}"/>
	       					</regprof_ext:extApplicationSpecialitiesView>	       					
        	...
        	<c:if test="${has_experience}">      	
	        	<fieldset><legend class="noForm">Информация за стаж</legend>
	        		<regprof_ext:extApplicationReportExperienceView attributePrefix="${attributePrefix}">
	        			<nacid_ext:report_subelement name="Професия" value="${profession}"/>
	        			<nacid_ext:report_subelement name="Професия(ново)" value="${profession_txt}"/>
	        			<fieldset><legend class="noForm">Документи</legend>
		        			<regprof_ext:extApplicationExperienceDocumentView attributePrefix="${attributePrefix}">	        			
		       					<fieldset><legend class="noForm">Документ</legend>
		       						...
		       						<fieldset><legend class="noForm">Дати</legend>     							
		       							<regprof_ext:extApplicationExperienceDatesView attributePrefix="${attributePrefix}" documentId="${doc_id}">						
		       								<nacid_ext:report_subelement name="Работил за периода" value="${single_date}"/>
		       							</regprof_ext:extApplicationExperienceDatesView>
		       						</fieldset>
		       					</fieldset>	        			
		        			</regprof_ext:extApplicationExperienceDocumentView>
	        			</fieldset>
	        			<div class="clr10"><!--  --></div>
	        		...
	        		</regprof_ext:extApplicationReportExperienceView>
	        	</fieldset>
		   ...
		   <fieldset><legend class="noForm">Професионална квалификация, за която се иска удостоверение</legend>
			   ...
			   <nacid_ext:report_subelement name="Професионална квалификация(ново)" value="${certificate_prof_qual_txt}"/>
		   </fieldset>
       </regprof_ext:extApplicationReportTrainingCourseView>
       
       <c:if test="${show_application_attachments && not hideAttachments}">
            <fieldset><legend class="noForm">Прикачени документи</legend>
            <regprof_ext:extApplicationAttachmentsList attributePrefix="${attributePrefix}">
                <a target="_blank" href="${attachmentPrefix }/?id=${id }&amp;att_type=${type}&amp;application_id=${requestScope[appl_Id]}" >${document_type }</a><br />
            </regprof_ext:extApplicationAttachmentsList>
            </fieldset>
        </c:if>
        <fieldset><legend class="noForm">Данни за плащане</legend>
        	<regprof_ext:extApplicationLiabilityView>
        	<nacid_ext:report_subelement name="Статус на плащане" value="${liability_status}" />
        	<nacid_ext:report_subelement name="Сума за плащане" value="${liability_amount}" />
        	<nacid_ext:report_subelement name="Дата на плащане" value="${liability_payment_date}" />
        	<nacid_ext:report_subelement name="Начин на плащане" value="${liability_payment_type}" />
        	</regprof_ext:extApplicationLiabilityView>
        </fieldset>
        <regprof_ext:extRegprofApplicationReportESignInformation>
...
</regprof_ext:extApplicationReportView>